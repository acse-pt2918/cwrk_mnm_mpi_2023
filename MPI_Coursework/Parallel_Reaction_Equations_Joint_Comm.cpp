...
void do_iteration_periodic(void)
{
	...
	vector<double*> received = { C1_old.start_pointer(), C2_old.start_pointer()};

	// Create temporary datatypes for combining C1 and C2 into a single message
	...
	type_list.push_back(CMatrix::MPI_Types[(int)direction::up_out]); // C1
	type_list.push_back(CMatrix::MPI_Types[(int)direction::up_out]); // C2
	block_lengths.push_back(1);
	block_lengths.push_back(1);

	MPI_Get_address(C1_old.start_pointer(), &address);
	displacements.push_back(address);
	MPI_Get_address(C2_old.start_pointer(), &address);
	displacements.push_back(address);

	MPI_Datatype temp_type;
	MPI_Type_create_struct(displacements.size(), block_lengths.data(), displacements.data(), type_list.data(), &temp_type);
	MPI_Type_commit(&temp_type);

	cout << "Completed datatype creation";
	cout.flush();

	// Periodic communication to get and send the boundary values, ONLY UP COMM IMPLEMENTED
	MPI_Isend(MPI_BOTTOM, 1, temp_type, neighbors_id[0], 1, MPI_COMM_WORLD, &request[request_cnt]);
	request_cnt++;
	cout << "Started sent";
	cout.flush();
	...
	MPI_Probe(neighbors_id[3], 1, MPI_COMM_WORLD, &status);

	MPI_Get_count(&status, CMatrix::MPI_Types[(int)direction::up_out], &count);

	MPI_Irecv(received.data(), count, CMatrix::MPI_Types[(int)direction::down_in], neighbors_id[3], 1, MPI_COMM_WORLD, &request[request_cnt]);
	request_cnt++;
	...
	MPI_Irecv(C1_old.start_pointer(), 2, CMatrix::MPI_Types[(int)direction::up_in], neighbors_id[0], 2, MPI_COMM_WORLD, &request[request_cnt]);
	request_cnt++;
	...
	// Save file after comm to check if the values are correct
	C1.swap_data(C1_old);
	C2.swap_data(C2_old);
	grid_to_file(5000);
	C1.swap_data(C1_old);
